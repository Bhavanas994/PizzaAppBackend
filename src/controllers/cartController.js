const { getCart ,modifyCart, clearProdutsFromCart } = require("../services/cartService");
...
async function clearCartById(req,res){
  try{
     const cart = await clearProdutsFromCart(req.user.id);
     return res.status(200).json ({
        success: true,
        message: "Successfully cleared all products from cart",
        data: cart,
        error: {}
